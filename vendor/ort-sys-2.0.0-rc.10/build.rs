...
fn static_link_prerequisites(using_pyke_libs: bool) {
	...
	if target_triple.contains("apple") {
		...
		if let Some(dir) = macos_rtlib_search_dir() {
			...
			// PATCH (Matrix): the upstream crate hardcodes the macOS compiler-rt
			// (`clang_rt.osx`) for every Apple target, which fails to link into an iOS
			// binary ("built for zippered(macOS/Catalyst)"). Pick the right variant.
			let rtlib = if target_triple.contains("ios") {
				if target_triple.contains("sim") { "clang_rt.iossim" } else { "clang_rt.ios" }
			} else {
				"clang_rt.osx"
			};
			println!("cargo:rustc-link-lib={rtlib}");
		}
	}
...
#[allow(dead_code)] // unused when building against a system lib (ORT_LIB_LOCATION)
